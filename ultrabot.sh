...
# Функция для сохранения учетных данных Marzban в JSON
...
# Функция для сохранения данных Outline в JSON
...
# Функция для установки Outline VPN
install_outline() {
    colorized_echo blue "Начинаем установку Outline VPN..."
    
    # Временный файл для сохранения вывода установки
    local temp_output="/tmp/outline_install_output.txt"
    
    # Запускаем установку и сохраняем вывод
    ...
    # Извлекаем API URL и cert SHA из вывода
...
# Функция для обратного отсчета и предупреждения
warning_countdown() {
...
# Функция для проверки и удаления существующих компонентов
cleanup_existing_installation() {
    colorized_echo yellow "Проверка существующей установки..."
    
    # Проверка и удаление исполняемого файла marzban
    ...
    # Проверка и удаление директорий
    if [ -d "$APP_DIR" ]; then
    ...
    # Остановка и удаление docker контейнеров если они существуют
...
get_keys() {
    ...
    # Пробуем первый вариант команды
    if ! output=$(docker exec marzban_marzban_1 xray x25519 2>/dev/null); then
        # Если произошла ошибка, пробуем второй вариант
        output=$(docker exec marzban-marzban-1 xray x25519)
    fi

    # Извлекаем public key из вывода (берем строку после "Public key: ")
    public_key=$(echo "$output" | grep "Public key:" | cut -d' ' -f3)

    # Записываем public key в файл pubkey.json
    echo "$public_key" > "$DATA_DIR/pubkey.json"

    # Генерируем случайный shortid и записываем в файл
...
create_config() {
    ...
    # Создаем JSON конфигурацию с подстановкой значений
...
restart_container() {
    ...
    if ! docker restart marzban_marzban_1 2>/dev/null; then
        # Если первое имя контейнера не сработало, пробуем второе
...
# Запускаем предупреждение и ждем
warning_countdown

# Запускаем очистку перед установкой
cleanup_existing_installation
...
# Сохраняем учетные данные в JSON файл
save_marzban_credentials

# После установки Marzban настраиваем REALITY
...
# После настройки REALITY устанавливаем Outline
colorized_echo blue "Начинаем установку Outline VPN..."
install_outline
